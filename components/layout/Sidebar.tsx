...
const SidebarLink: React.FC<SidebarLinkProps> = ({
  ...
  <button
    id={id}
    onClick={onClick}
    className={`w-full flex items-center px-4 py-3 rounded-xl transition-all duration-300 ease-out text-xs lg:text-sm font-semibold tracking-wide relative ${isCollapsed ? "justify-center space-x-0" : "space-x-3"
      } ${active
        ? isDarkTheme
  ...
  >
    <span
      className={`shrink-0 relative flex items-center justify-center transition-opacity duration-300 ${active ? "opacity-100" : "opacity-60"
        }`}
    >
      {icon}
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`border-r flex flex-col shrink-0 transition-all duration-500 ease-out z-50 ${isCollapsed
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-8 px-4 space-y-8">
          ...
          <div id="driver-active-tasks-section">
            {!isCollapsed && (
              <div className="flex items-center justify-between px-4 mb-4 animate-in fade-in duration-500">
                <span
                  className={`text-[10px] lg:text-xs font-semibold tracking-wide ${isDarkTheme ? "text-white/60" : "text-gray-700"
                    }`}
                >
                  {labels.activeTasks}
                </span>
              ...
              {tasks.map((task) => (
                ...
                >
                  ...
                  >
                    ...
                    {!isCollapsed && (
                      <span
                        className={`text-xs lg:text-sm font-semibold truncate min-w-0 transition-colors duration-300 ${activeTaskId === task.id
                          ? isDarkTheme
                            ? "text-white/90"
                            : "text-gray-900"
                          : isDarkTheme
                  ...
                  {!isCollapsed && (
                    <div className="flex items-center space-x-2 flex-shrink-0 ml-2">
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-all duration-300 opacity-0 group-hover:opacity-100 flex-shrink-0 hover:scale-110 ${isDarkTheme
          ...
          <div>
            {!isCollapsed && (
              <span
                className={`text-[10px] lg:text-xs font-semibold tracking-wide px-4 block mb-4 animate-in fade-in duration-500 ${isDarkTheme ? "text-white/60" : "text-gray-700"
                  }`}
              >
                {labels.options}
              </span>
        ...
        <div
          className={`p-6 border-t text-[10px] lg:text-xs font-semibold tracking-wide text-center transition-colors duration-300 ${isDarkTheme
