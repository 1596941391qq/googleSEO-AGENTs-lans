// EdgeOne Functions - Gemini API 服务
// 使用环境变量配置 (在 EdgeOne 控制台设置)
...
// 从环境变量获取配置 (EdgeOne 支持 globalThis.env 或直接环境变量)
function getEnv(key, defaultValue) {
  // EdgeOne Functions 环境变量获取方式
...
const PROXY_BASE_URL = getEnv('GEMINI_PROXY_URL', 'https://api.302.ai');
const API_KEY = getEnv('GEMINI_API_KEY', 'sk-BMlZyFmI7p2DVrv53P0WOiigC4H6fcgYTevils2nXkW0Wv9s');
const MODEL = getEnv('GEMINI_MODEL', 'gemini-2.5-flash');

async function callGeminiAPI(prompt, systemInstruction, config) {
...
export const translatePromptToSystemInstruction = async (userPrompt) => {
...
export const analyzeRankingProbability = async (keywords, systemInstruction) => {
  const analyzeSingleKeyword = async (keywordData) => {
...
export const generateDeepDiveStrategy = async (keyword, uiLanguage, targetLanguage) => {
