// EdgeOne Functions - Gemini API 服务 (TypeScript版本)
...
async function callGeminiAPI(
  ...
  const proxyUrl = env.GEMINI_PROXY_URL || 'https://api.302.ai';
  const apiKey = env.GEMINI_API_KEY;
  const model = env.GEMINI_MODEL || 'gemini-2.5-flash';
  
  const url = `${proxyUrl}/v1/v1beta/models/${config?.model || model}:generateContent`;
...
export const translatePromptToSystemInstruction = async (userPrompt: string, env: Env): Promise<string> => {
...
export const translateText = async (text: string, targetLanguage: 'zh' | 'en', env: Env): Promise<string> => {
