...
1. ~~**蓝海信号评分算法**~~ ✅ **已实现** - 已改为显式评分算法，包含详细打分原因
...
**1. 工作流 1+3 (关键词挖掘) - 蓝海信号评分** ✅ **已实现**

- 文件: `api/_shared/agents/agent-2-seo-researcher.ts`
- 功能: 新增 `calculateBlueOceanScore()` 函数，返回详细的分值分解
- 替代: AI 黑盒判断 → 显式评分算法 + 详细打分原因
- 评分规则:
  - 弱竞争者类型 (Forum/Social 或 Weak Page): +30
  - 内容相关性低: +25
  - 内容质量不足: +20
  - 搜索结果数量较少 (<10000): +15
  - 搜索结果数量中等 (10000-100000): +10
  - 无直接竞争对手 (serpResultCount = 0): +20
  - 关键词难度极低 (KD <= 20): +15
  - 关键词难度较低 (KD <= 40): +5
  - 总分 >= 70: HIGH, >= 40: MEDIUM, <40: LOW
- UI 展示:
  - ✅ 在 KeywordTable 展开行中显示蓝海分数详细分解（每个维度的得分和原因）
  - ✅ 在 Agent 过程可视化中显示蓝海分数详细分解
  - ✅ 蓝海模式隐藏 DR 对比列（仅在存量拓新模式显示）
