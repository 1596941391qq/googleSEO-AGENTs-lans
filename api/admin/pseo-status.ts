...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const [
      ...
      sql`SELECT COUNT(*) as count FROM platform_tokens_v2`,
      sql`SELECT COUNT(*) as count FROM platform_sites_v2`,
    ...
    const [githubTokens, platformTokens, sites] = await Promise.all([
      ...
      sql`SELECT id, platform, name, usage_count, status, created_at FROM platform_tokens_v2 ORDER BY created_at DESC`,
      sql`
        SELECT 
